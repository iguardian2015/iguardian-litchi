...
public class MainActivity extends ActionBarActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (login != 1) {
			...
			// List<String> user_password = datasource.get_user_password();
			datasource.close();
			/*
						...
						public void onItemSelected(AdapterView<?> parent,
							...
							if (check > 1) {
								...
								List<String> school = datasource
										.get_school_db();
								List<String> user_password = datasource
										.get_user_password();
								datasource.close();
								// Log.i("Value of ", "good2");
								dbSelected = school.get(index);
								passSelected = user_password.get(index);

								session.createLoginSession(
										user_selected.toString(), passSelected,
										dbSelected);
	...
	void login() { // Re-authenticating every time on start of the application.
		...
		try {
			...
			if (response3.equalsIgnoreCase("User Found")) {
				...
				/*frgManager.beginTransaction()
				.add(R.id.mainContent, fragment).addToBackStack(null)
				.commit();*/
	...
	public void password_changed_alert() {
		MainActivity.this.runOnUiThread(new Runnable() {
			public void run() {
				...
				builder.setMessage("It seems that password has been changed.")
						.setCancelable(false)
						.setPositiveButton("OK",
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog,
