...
public class LoginActivity extends Activity {
	...
	void login() {
		...
		try {
			...
			final String response2 = httpclient.execute(httppost,
					responseHandler);
			System.out.println("Response : " + response2);
			dbSelected = response2 ;
		...
		try { /*
			...
			if (response3.equalsIgnoreCase("User Found")) {
				Log.i("Response : ", "done1");
				//response.getEntity().consumeContent();
				Log.i("Response : ", "done2");
				// tv.setText("vhsjdskdi");
				session.createLoginSession(et.getText().toString().trim(), pass.getText().toString().trim(),
						dbSelected.trim());
				
				Log.i("Response : ", "done3");
				datasource.createComment(et.getText().toString().trim(),
						dbSelected.trim(),  pass.getText().toString().trim());
				Log.i("Response : ", "done4");
